---
title: "ANCAT Analysis: 19-month-olds"
output:
  html_document:
    toc: true
    theme: united
...
model <- lmer(ArcSin ~ TargetC + (1 + TargetC | Trial) + (1 + TargetC | ParticipantName), data = familiar_trial, REML = FALSE)
summary(model)
drop1(model,~.,test="Chi")

# logistic regression (experimental here)
model_experimental <- glmer(cbind(SamplesInAOI, (SamplesTotal - SamplesInAOI)) ~ TargetC + (1 + TargetC | Trial) + (1 + TargetC | ParticipantName), data = familiar_trial, family="binomial")
summary(model_experimental)
```
...
onset_switches$FirstAOIC <- onset_switches$FirstAOIC - mean(onset_switches$FirstAOIC, na.rm=T)
...
# logistic regression (experimental here)
model_logistic <- glmer(cbind(SamplesInAOI, (SamplesTotal - SamplesInAOI)) ~ ConditionC + (1 | Trial) + (1 | ParticipantName), data = unfamiliar_trial, family="binomial")
summary(model_logistic)
drop1(model_logistic,~.,test="Chi")
```
...
```{r}
familiar_divergence <- subset_by_window(familiar, window_start_time = 400, window_end_time = 5490, rezero=TRUE, remove=TRUE)
...
unfamiliar_divergence <- subset_by_window(unfamiliar, window_start_time = 3140, window_end_time = 4060, rezero=TRUE, remove=TRUE)
...
write.csv(familiar_divergence_agg,'familiar_divergence_agg_19.csv')
write.csv(unfamiliar_divergence_agg,'unfamiliar_divergence_agg_19.csv')
```