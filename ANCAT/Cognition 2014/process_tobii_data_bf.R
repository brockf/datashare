...
auto_process_tobii <- function (aoifile, phasefile, only_condition = FALSE, only_trialorder = FALSE, condition_regex = "_[a-zA-Z0-9]{3,}_") {
  ...
  for (file in files) {
    ...
    # get condition
    conditionLocation = regexpr(condition_regex, file)
...
process_tobii <- function(tobiifile, aoifile, phasefile, condition, trialorder, outputfile=NULL) {
  ...
  # Sometimes we load an empty column at the end, let's see if that's the case and
  # remove it
  if (length(colnames(tobii)) >= 30 & colnames(tobii)[30] == 'X') {
    tobii <- tobii[, 1:29]
  }
  ...
  if ('GazePointLeftX..ADCSpx.' %in% colnames(tobii)) { setnames(tobii, 'GazePointLeftX..ADCSpx.', 'GazePointLeftX') }
  if ('GazePointRightX..ADCSpx.' %in% colnames(tobii)) { setnames(tobii, 'GazePointRightX..ADCSpx.', 'GazePointRightX') }
  if ('GazePointLeftY..ADCSpx.' %in% colnames(tobii)) { setnames(tobii, 'GazePointLeftY..ADCSpx.', 'GazePointLeftY') }
  if ('GazePointRightY..ADCSpx.' %in% colnames(tobii)) { setnames(tobii, 'GazePointRightY..ADCSpx.', 'GazePointRightY') }
  if ('GazePointY..ADCSpx.' %in% colnames(tobii)) { setnames(tobii, 'GazePointY..ADCSpx.', 'GazePointY') }
  if ('GazePointX..ADCSpx.' %in% colnames(tobii)) { setnames(tobii, 'GazePointX..ADCSpx.', 'GazePointX') }
  if ('X.StudioTestName' %in% colnames(tobii)) { setnames(tobii, 'X.StudioTestName', 'StudioTest') }
  if ('X.Gender.Value' %in% colnames(tobii)) { setnames(tobii, 'X.Gender.Value', 'Gender') }
  
  # Rename the "MediaName" column to "MovieName" to reduce ambiguity
  # (note from Brock: what ambiguity?)
  if ('MediaName' %in% colnames(tobii)) { setnames(tobii, 'MediaName', 'MovieName') }
  ...
  # use dplyr's left_join for speed increase
  # but we need to rename MovieName column to Filename for the merge, then rename it back
  #tobii <- merge(x=tobii, y=phaseTimingSubset, by.x="MovieName", by.y="Filename", all.x=TRUE)
  
  setnames(tobii, 'MovieName','Filename')
  tobii <- left_join(x=tobii, y=phaseTimingSubset, by=c("Filename"))
  setnames(tobii, 'Filename','MovieName')
  
  # The merge could have messed up the order of the data, so sort by timestamp.
  #tobii <- tobii[order(tobii$RecordingTimestamp), ]
  tobii <- arrange(tobii, RecordingTimestamp)
  ...
  # make sure columns are proper types so we avoid warnings and issues later on
  # (like trying to do math with factors)
  ...
  for (col_name in numeric_columns) {
    if (class(tobii[, col_name]) != "numeric") {
      cat(paste("... Warning: ", col_name, " is not a numeric column. We'll attempt to convert it in a moment...\n",sep=""))
      tobii[, col_name] <- as.numeric(as.character(tobii[, col_name]))
    }
  }
...
prepare_master <- function (subjectfile = FALSE, trialfile = FALSE, inputfile='master.csv', outputfile='master-prepared.csv') {
  ...
  if (subjectfile != FALSE) {
    ...
    if (is.na(match("ParticipantName", names(subjects)))) {
      ...
      # use dplyr's left_join for speed
      #tobii <- merge(tobii, subjects, by=c("ParticipantName"), all.x = TRUE)
      tobii <- left_join(tobii, subjects, by=c("ParticipantName"))
    }
  }
  ...
  if (trialfile != FALSE) {
    ...
    if (is.na(match("Trial", names(trials)))) {
      ...
      tobii <- left_join(tobii, trials, by=c("Trial"))
    }
  }
...
#
convert_files <- function (sub_folder, filename_prefix = 'TNV24M_', condition_regex = "M[a-zA-Z]+_[A-Z]", participant_number_regex = "Rec [0-9]+") {
  library(data.table, quietly=T)
  ...
  for (file in files) {
    ...
    for (i in 1:nrow(tobii)) {
      # format the column names (some files will load with the first row as the column names - we don't want that)
      colnames(tobii) <- c('V1','V2')
    ...
    tobii <- read.table(file=paste(sub_folder,'/',file,sep=""), sep="\t", header=TRUE, row.names = NULL, fill=TRUE, skip=import_skip_headers, strip.white = T)
    
    # sometimes files are improperly formatted and need to be opened and re-saved in Excel
    # when this is a problem, we'll know because we'll only have 2 columns imported
    if (ncol(tobii) < 10) {
      stop("Error loading in data file. Please open and re-save as a tab-separated values (TSV) file in Excel.")
    }
    
    # now make appropriate formatting changes
    ...
    tobii$StudioTestName <- trim(condition)
    tobii$ParticipantName <- trim(as.character(participant))
    tobii$Gender <- trim(gender)
    tobii$RecordingName <- trim(as.character(recording_name))
    tobii$RecordingDate <- trim(as.character(date))
    ...
    setnames(tobii_new, c('StudioTest','ParticipantName','Gender','RecordingName','RecordingDate','RecordingDuration','MediaName','RecordingTimestamp','LocalTimeStamp','StudioEvent','FixationIndex','SaccadeIndex','GazeEventType','GazeEventDuration','FixationPointX','FixationPointY','AOI[Left AOI]Hit','AOI[Right AOI]Hit','GazePointIndex','GazePointLeftX','GazePointLeftY','GazePointRightX','GazePointRightY','GazePointX','GazePointY','PupilLeft','PupilRight','ValidityLeft','ValidityRight'))
    
    # write new file
    participantNumberLocation = regexpr(participant_number_regex, file)
    ...
    participantNumber = gsub("[^0-9]","", participantNumber)
    
    filename <- paste(filename_prefix, condition, '_', participantNumber,'.tsv',sep="")
    cat(paste("Saving file as '",filename,"... ",sep=""))
    write.table(tobii_new, filename, sep = "\t", row.names = F)
    cat("Saved.\n\n")
  }
}
